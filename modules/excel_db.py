...
class ExcelToDb(object):
    ...
    def __init__(self, xl_file_name, db_name):
        """
        ...
        create_statement = self.createStatement(res)

        print("Creating the insert query statements")
        insert_statements = self.insertStatements(res)

        print("Inserting into the DB")
        self.dataBaseInteract(db_name, res, create_statement, insert_statements)

    def dataBaseInteract(self, db_name, res, create_statement, insert_statements):
    ...
    def createStatement(self, res):
        ...
        for col_name in res["col_names"]:
            create_table_statement += str(col_name) + " " + self.dtypeDB(res['row_data'][0][index])
            index += 1
    ...
    def readData(self, xl_file_name):
